...
/// checks the time, runs a sandboxed shell command, then summarizes.
/// </summary>
internal sealed class OfflineChatClient : IChatClient
{
    ...
    )
    {
        ...
        FunctionCallContent shellCall = new(
            "offline-2",
            "run_shell_command",
            new Dictionary<string, object?> { ["command"] = "echo Hello from the Orkis sandbox; pwd" }
        );
        ...
        var response = _turn switch
        {
            ...
            _ => Scripted("Done: I checked the time and ran the command. (offline scripted response)"),
        };
